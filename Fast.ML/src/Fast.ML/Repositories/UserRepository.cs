...
public class UserRepository : IUserRepository
{
    ...
    {
        var parameters = new {Email = email, FirstName = firstName, LastName = lastName};
        using var connection = _context.CreateConnection();
    ...
    private const string GetUserIdQuery = "select id from fastml.dbo.users where email = @Email";

    private const string CreateUserQuery =
        "insert into fastml.dbo.users (email, first_name, last_name) values (@Email, @FirstName, @LastName)";
