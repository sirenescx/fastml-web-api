...
public class AlgorithmRepository : IAlgorithmRepository
{
    ...
    public async Task<IEnumerable<Algorithm>> GetAlgorithmsAsync()
    {
        ...
        var x = connection.QuerySingleAsync("select * from fastml.dbo.algorithms where id = 1").Result;
        var y = connection.QueryAsync(GetAlgorithmsQuery).Result;
    ...
    public async Task<Endpoint> GetEndpointByAlgorithmAsync(Algorithm algorithm)
    {
        ...
        using var connection = _context.CreateConnection();
        return await connection.QuerySingleOrDefaultAsync<Endpoint>(GetEndpointByAlgorithmQuery, parameters);
    }
    ...
    private const string GetEndpointByAlgorithmQuery = 
        @"select * from fastml.dbo.endpoints 
            join fastml.dbo.algorithms_endpoints on endpoints.id = algorithms_endpoints.endpoint_id 
